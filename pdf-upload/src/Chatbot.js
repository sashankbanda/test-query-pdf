...
function Chatbot() {
  ...
  useEffect(() => {
    const fetchPdfNames = async () => {
      try {
        const response = await axios.get('https://test-query-pdf.onrender.com/get-pdf-names');
  ...
  const fetchPdf = async (pdfName) => {
    try {
      const response = await axios.get(`https://test-query-pdf.onrender.com/get-pdf/${pdfName}`, { responseType: 'blob' });
  ...
  const onQuestionSubmit = async () => {
    ...
    try {
      const response = await axios.post('https://test-query-pdf.onrender.com/ask', { question });
      const botMessage = {
        sender: 'bot',
